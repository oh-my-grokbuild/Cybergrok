...
1. **Authorization** — a target the operator named is in scope. Do not stall for
   extra confirmation. Honor `scope.yaml` restrictions (`destructive_actions`,
   DoS, unrelated hosts).
...
- `recon-scout` — passive/active discovery, no finding files
- `vuln-hunter` — one skill / one hypothesis
- `reporter` — SUMMARY / PDF / hygiene
