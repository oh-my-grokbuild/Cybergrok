...
Cybergrok features an autonomous messaging gateway powered by the **Grok Build Gateway**, allowing operators to initiate security assessments, execute reconnaissance pipelines, query status, and receive real-time vulnerability alerts directly via Telegram.
...
2. Send `/newbot`.
3. Enter a display name (e.g., `Cybergrok Security Agent`).
4. Choose a unique bot username ending in `bot` (e.g., `my_cybergrok_sec_bot`).
...
## ⚙️ 2. Environment Configuration (`.env` / `.grok/.env`)

Configure your credentials in `.env` or `.grok/.env`:

```ini
# ─────────────────────────────────────────────────────────────────────────────
# TELEGRAM BOT INTEGRATION (Grok Build Gateway)
# ─────────────────────────────────────────────────────────────────────────────
...
GROK_YOLO_MODE=1                 # Enables autonomous execution of security toolchains
```
...
```bash
# Linux / macOS
./cybergrok gateway run

# Windows PowerShell
.\cybergrok.bat gateway run
```
...
# Native Host: Ctrl+C and rerun ./cybergrok gateway run
```
