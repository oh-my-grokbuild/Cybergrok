# 🗺️ Cybergrok MCP Server — Roadmap & Architecture Status

This document records the completed milestones, active architectural capabilities, and future engineering roadmap for the **Cybergrok Model Context Protocol (MCP) Server**.
...
| :--- | :--- | :--- |
| **Tool** | `cybergrok_validate_scope` | Validates target domain/URL/IP against `scope.yaml` (Wildcard `*.target.com`, CIDR `192.168.1.0/24`, path exclusions). |
| **Tool** | `cybergrok_http_probe` | HTTP inspection, TLS cert extraction, and framework fingerprinting (Next.js, Laravel, Spring Boot, WordPress, etc.). Dual engine (`httpx` + Go `net/http`). |
| **Tool** | `cybergrok_recon_crawl` | Endpoint discovery & JS bundle mining with Smart Pipe token budgeting (top 25 high-signal routes in context) + raw dump preservation. |
| **Tool** | `cybergrok_search_knowledge` | Sub-50ms search across 50,000+ curated payloads (PayloadsAllTheThings, HackTricks, Strix, Claude-BugHunter). |
| **Tool** | `cybergrok_list_skills` | Catalog and filter 200+ offensive security playbooks. |
| **Tool** | `cybergrok_get_skill` | Retrieve complete Markdown SOP playbooks or specific section headings. |
| **Tool** | `cybergrok_scan_secrets` | 48-pattern credential leak detector (AWS, GCP, GitHub, Slack, Private Keys, JWTs) with automated masking. |
| **Tool** | `cybergrok_record_finding` | Record verified findings to `reports/<target>/findings/` matching strict `AGENTS.md` format with standalone PoC scripts. |
| **Tool** | `cybergrok_aggregate_report` | Executive summary generator aggregating metrics into `SUMMARY.md` and `metadata.json`. |
...
- **Universal AI Client Integration**: Comprehensive setup guides and auto-injection support for 11+ AI clients: OpenCode, Cursor IDE, Claude Desktop, Windsurf, Cline, Roo Code, Claude Code CLI, Continue.dev, Zed, Kilo Code, Grok Build, Codex, and Google Antigravity in [`docs/MCP_SETUP.md`](MCP_SETUP.md).
