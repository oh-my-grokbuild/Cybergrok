...
This guide provides step-by-step instructions for running **Cybergrok** on **Windows 10 / 11**.
...
### Step 3: Clone Cybergrok Repository
...
> ```
> *(Tip: Choosing a short root directory such as `C:\Cybergrok` or `C:\src\Cybergrok` also prevents path issues).*
...
- Prepare `.env` and `.grok/config.yaml`.
...
>   ```
> - If Windows Defender blocks research payloads, add the Cybergrok folder to exclusions:
...
```powershell
docker compose exec grok-cybergrok grok "Assess http://127.0.0.1:8888"
```
...
**Fix:** Add the Cybergrok project directory to your Windows Defender exclusions:
- Windows Security -> *Virus & threat protection* -> *Manage settings* -> *Exclusions* -> *Add an exclusion (Folder)* -> Select the `Cybergrok` root directory.
